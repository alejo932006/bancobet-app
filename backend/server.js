...
// ==========================================
// --- MÓDULO INDEPENDIENTE: RECARGAS BETPLAY ---
// ==========================================

// 1. Cliente: Crear nueva solicitud de recarga aislada
app.post('/api/betplay-recargas', async (req, res) => {
    const { usuario_id, monto, numero_destino } = req.body;
    try {
        const result = await pool.query(
            `INSERT INTO recargas_betplay (usuario_id, monto, numero_destino, estado) 
             VALUES ($1, $2, $3, 'pendiente') RETURNING *`,
            [usuario_id, monto, numero_destino]
        );
        res.json({ success: true, data: result.rows[0], message: "Recarga solicitada correctamente" });
    } catch (err) {
        console.error("Error en recarga betplay:", err);
        res.status(500).json({ success: false, error: err.message });
    }
});

// 2. Admin: Obtener todas las recargas (con filtro de fecha para el reporte)
app.get('/api/admin/betplay-recargas', async (req, res) => {
    const { fechaInicio, fechaFin } = req.query;
    try {
        let query = `
            SELECT r.*, u.nombre_completo, u.cedula 
            FROM recargas_betplay r
            JOIN usuarios u ON r.usuario_id = u.id
            WHERE 1=1
        `;
        const params = [];
        
        // Usamos el mismo ajuste de zona horaria (- 5 hours) que ya tienes en el sistema
        if (fechaInicio && fechaFin) {
            query += ` AND (r.fecha_solicitud - INTERVAL '5 hours')::date BETWEEN $1 AND $2`;
            params.push(fechaInicio, fechaFin);
        }
        ...
        console.error("Error obteniendo recargas betplay:", err);
        res.status(500).json({ error: err.message });
    }
});

// 3. Admin: Actualizar estado de la recarga (Aprobada, Rechazada)
app.put('/api/admin/betplay-recargas/:id', async (req, res) => {
    const { id } = req.params;
    const { estado } = req.body; // Recibirá 'aprobada' o 'rechazada'
    try {
        await pool.query('UPDATE recargas_betplay SET estado = $1 WHERE id = $2', [estado, id]);
