...
async function resetearSaldoKairo() {
    const confirm = await Swal.fire({
        ...
        text: "Esto calibrará internamente el saldo a $0 sin afectar el historial.",
        icon: 'question',
        showCancelButton: true,
    ...
    if (confirm.isConfirmed) {
        // Mostrar modal de carga para evitar que el usuario toque otras cosas
        Swal.fire({ title: 'Calibrando...', allowOutsideClick: false, didOpen: () => { Swal.showLoading() } });
        
        try {
            // 1. Obtener datos actuales SIN CACHÉ (_t=Date.now())
            const resResumen = await fetch(`${API_URL}/resumen?_t=${Date.now()}`);
            const dataResumen = await resResumen.json();
            const saldoActual = parseFloat(dataResumen.kairo.saldo) || 0;

            // 2. Obtener el ajuste actual sin caché
            const resAjuste = await fetch(`${API_URL}/config-kairo?_t=${Date.now()}`);
            ...
            const resGuardar = await fetch(`${API_URL}/config-kairo`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
            ...
            if (resGuardar.ok) {
                Swal.fire('Éxito', 'El saldo global de Kairoplay está calibrado en $0.', 'success');
                
                // [SOLUCIÓN 1]: Limpiar los filtros de fecha para ver el Saldo Global real
                document.getElementById('dash-inicio').value = '';
                document.getElementById('dash-fin').value = '';
                document.getElementById('dash-rango-texto').innerText = 'Mostrando actividad global';
                
                // [SOLUCIÓN 2]: Forzar el cambio visual a $0 inmediatamente
                const saldoEl = document.getElementById('kairo-saldo');
                if(saldoEl) {
                    saldoEl.innerText = '$ 0';
                    saldoEl.className = "text-2xl font-bold mt-1 text-gray-800"; // Quitar colores rojo/verde
                }
                
                // [SOLUCIÓN 3]: Recargar la data real desde el servidor
                cargarResumen(); 
                cargarAjusteKairo(); 
            } else {
